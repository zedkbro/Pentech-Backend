...
import Award from '../../models/admins/Award.js';
import AdminService from '../../services/adminService.js';

const service = new AdminService(Award);

class AwardController extends SuperController {
  constructor(service) {
    super(service);
  ...
  async create(req, res) {
    ...
    try{
      const { error, value } = validator.validateAwards(req.body);
      if(error){
  ...
  async getLatest(req, res){
    ...
    try {
      const result =  await service.findLatest(orderedBy);
      if (!result) {
  ...
  async updateById(req, res) {
    ...
    try{
      if (req.file && imageField) {
          const uploadedFile = req.file;
          req.body[imageField] = uploadedFile.filename;
          const existingEntity = await service.findById(req.params.id); 
          if (existingEntity && existingEntity[imageField]) {
              await deleteImage(existingEntity[imageField]);
          }
      }   
...
export default new AwardController(service);
